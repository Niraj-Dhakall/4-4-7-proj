...
export default function ProfilePage() {
    ...
    const fetchStudentData = async (studentId: string) => {
        try {
            const response = await fetch(`/api/students/getStudentByID?id=${studentId}`)
            if (response.ok) {
                    ...
                    <div className="flex justify-start w-full bg-gray-200 p-5 ">
                        ...
                        <div className="flex flex-col items-baseline">
                            <div className=" ml-5 mt-2">
                                <h1 className="font-bold text-xl text-black">{student.name}</h1>

                            </div>
                            <div className="mt-5">
                                <h1 className="text-sm text-slate-500"> Email:<span className="text-black font-semibold"> {student.email}</span></h1>
                                <h1 className="text-sm text-slate-500"> GPA:<span className="text-black font-semibold"> {student.gpa}</span></h1>
                            </div>
                        </div>
                    </div>
                    {/* degree stuff */}
                    ...
                    <div className="flex flex-col w-full justify-start p-5 ">
                        ...
                        <div className="max-w-7xl md:max-w-5xl bg-gray-200/20 p-5">
                            {student.major.length === 0 ? (
                                <p className="text-slate-400">No major listed</p>
                            ) : (
                                student.major.map((major, index) => (
                                    <div key={index} className="flex gap-2 items-baseline-center mb-3">
                                        <div className="flex gap-2">
                                            <div className={`${getColorFromName(major.name)} w-15 h-10 justify-center items-center flex`}>
                                                <h2 className="font-bold text-md text-black">{translateDegree(major.level)}</h2>
                                            </div>
                                            <div className="flex flex-col">
                                                <h1 className="font-bold text-md text-slate-500">{major.name}</h1>
                                                <h1 className="font-bold text-xs text-slate-600">Graduation: {major.gradYear}</h1>
                                            </div>
                                        </div>
                                    </div>
                                ))
                        ...
                        <div>
                            ...
                            <div className="max-w-7xl md:max-w-5xl bg-gray-200/20 p-5">
                                {!student.portfolio ?
                                    <div>
                                        <h1 className="text-slate-400"> No portfolio listed</h1>
                                    </div>
                                    :
                                    <div className="flex items-baseline-start gap-2">
                                        <ChevronRightSquare className="text-black" /> <a href={student.portfolio}><span className="text-black text-md font-semibold">{student.portfolio}</span></a>
                                    ...
                                    (
                                        <div className="flex flex-wrap">
                                            {student.skills.map((skill, index) => (
                                                <div className="gap-2 ">
                                                    <div className="flex gap-2  items-baseline text-black" key={skill + index} >
                                                        <Circle className="text-black w-[10px] h-[10px]" /> {skill}
                                                    </div>
                                                </div>
